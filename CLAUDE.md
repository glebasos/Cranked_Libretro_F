...
A Windows/MSVC port of **Cranked** (a Playdate console emulator, originally by TheLogicMaster) focused on its **libretro core** for RetroArch, plus the SDL2/ImGui **desktop app** with debugging views. Lua games run well: graphics, text, sprites, file APIs, sample/file-player audio, and synth audio (oscillators + ADSR) all work. Native (ARM/pdex.bin) games use unicorn emulation and are less tested. Instrument/sequence sources and all effects except the bitcrusher are still stubs.
...
- **`CRANKED_AUDIO_DUMP=<path>`** writes the mixed audio out as raw s16le stereo PCM — the audio counterpart of the screenshot loop. Analyze with numpy: FFT for pitch, harmonic ratios to identify a waveform, distinct-value counts to prove quantization. "It makes noise" is not verification; check the frequency.
- To read a game's logic: unpack `.pdz` with the pdz tool from the playdate-reverse-engineering repo, then decompile `.luac` with scratchminer's unluac fork — output is near-perfect Lua. Note a game may not be what its name suggests — Satellite is a *synthesizer*, not a sampler (it ships zero audio files), which is why it was silent.
...
- `Audio` (`core/Audio.{hpp,cpp}`) — `Audio::sampleAudio` mixes `mainChannel` + `channels`. Each channel gets its own int32 bus: its sources sum into it, its effect chain processes that, then volume/pan applies and it sums into the master accumulator. Sources implement `SoundSource_32::sampleAudio` — `playSampleAudio` covers SamplePlayer/FilePlayer, `PDSynth_32::sampleAudio` is the oscillator+ADSR synth. FilePlayer buffers whole files via `loadSample` (no streaming). The libretro core pumps 735 frames per 60fps video frame at 44100 Hz; the desktop app uses SDL queue-mode audio from the main thread. **Everything is single-threaded** — audio mixing mutates player/voice state and must stay on the emulator thread.
...
- **.pft page usage flags are one bit per page, LSB-first: page = `byte * 8 + bit`** (was `byte * 64 + bit` in `Rom::readFontData`). Byte 0 makes both formulas agree, so pages 0–3 (all of ASCII/Latin/Greek) always worked and the bug was invisible for normal text — but every page above U+03FF was filed under a bogus index and its glyphs silently vanished (`layoutText` `continue`s on a lookup miss). Symptom: smolitaire's dialog buttons showed bare "Ok"/"Cancel" instead of "Ⓐ Ok"/"Ⓑ Cancel" (U+24B6/U+24B7, page 36), its Help screen lost the 🎣 crank glyph (page 499), and the title footer lost its "•" (U+2022, page 32) — the buttons look unlabeled rather than "unselected", because these dialogs have no selected button at all, they tell you which *button* to press. Both kerning tables were misparsed too: the short table keyed off the sequential page counter instead of the real page index, and the long entry is a **uint24 codepoint at offset 0 with the int8 kerning at offset 3** (it was reading `entry >> 8` as the key and the codepoint's low byte as the kerning). Same family as the `.pda` header bug — check the format docs in `playdate-reverse-engineering/formats/` before trusting a parser here.
